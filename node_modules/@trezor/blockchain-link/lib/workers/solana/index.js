...
const getAccountInfo = (request) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
    ...
    const getTransactionPage = (txIds, tokenAccountsInfos, tokenDetailByMint) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
        const transactionsPage = yield fetchTransactionPage(api, txIds);
        return (yield Promise.all(transactionsPage
            .filter(isValidTransaction)
            .map(tx => blockchain_link_utils_1.solanaUtils.transformTransaction(tx, payload.descriptor, tokenAccountsInfos, tokenDetailByMint)))).filter((tx) => !!tx);
    });
    ...
    const shouldFetchTokens = tokenAccounts.value.length > 0 &&
        (details === 'tokens' || details === 'txs' || details === 'tokenBalances');
    let tokens = [];
    let tokenDetailByMint = {};
    if (shouldFetchTokens) {
        tokenDetailByMint = yield (0, tokenUtils_1.fetchCoingeckoTokenDetailByMint)(tokenAccountsInfos.map(a => a.mint).filter((mint) => !!mint));
        tokens = (0, solana_1.transformTokenInfo)(tokenAccounts.value, tokenDetailByMint);
    }
    const transactionPage = details === 'txs'
        ? yield getTransactionPage(txIdPage, tokenAccountsInfos, tokenDetailByMint)
        : undefined;
