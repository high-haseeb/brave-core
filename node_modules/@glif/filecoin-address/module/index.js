...
export class Address {
  constructor(str, coinType = defaultCoinType) {
...
export function newAddress(protocol, payload, coinType = defaultCoinType) {
  const protocolByte = new Uint8Array([protocol]);
  return new Address(uint8arrays.concat([protocolByte, payload]), coinType);
}
export function newIDAddress(id, coinType = defaultCoinType) {
...
export function decode(address) {
  ...
  const coinType = address.slice(0, 1);
  ...
  if (encode(coinType, addressObj) !== address) throw Error(`Did not encode this address properly: ${address}`);
  return addressObj;
}
